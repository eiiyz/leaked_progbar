$('document').ready(function() {
    ...
    MythicProgBar.Progress = function(data) {
        ...
        $("#progress-label").text(data.label.toUpperCase());
        $("#progress-bar").stop().css({"width": 0, "background-color": "#rgba(80,135,177,0.9);"}).animate({
          width: '100%'
        }, {
          ...
          complete: function() {
            ...
            $.post('http://cfx_progbar/actionFinish', JSON.stringify({
                })
            );
          }
        });
    ...
    MythicProgBar.ProgressCancel = function() {
        ...
        setTimeout(function () {
            $(".progress-container").css({"display":"none"});
            $("#progress-bar").css("width", 0);
            $.post('http://cfx_progbar/actionCancel', JSON.stringify({
                })
            );
        }, 1000);
    };
